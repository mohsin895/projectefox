           ...
           $categoryCount= SubCategory::where('url',$url)->where('status','1')->count();
            //   echo "<pre>";print_r( $categoryCount);die();
                        if($categoryCount > 0){
                            ...
                            if (isset($_GET['start_price']) && $_GET['start_price'] && $_GET['end_price']) {
                                $start_pricedate =  $_GET['start_price'];
                                $end_pricedate =  $_GET['end_price'];
                               
                                $categoryAddvertisement->whereIn('advertisings.price', [$start_pricedate, $end_pricedate]);
                            }

                            if(isset($data['sell']) && !empty($data['sell'])){
                                $categoryAddvertisement->whereIn('advertisings.sell',$data['sell']);

                            }
                            if(isset($data['transmissions']) && !empty($data['transmissions'])){
                                $categoryAddvertisement->whereIn('advertisings.transmissions',$data['transmissions']);

                            }

                            if(isset($data['door']) && !empty($data['door'])){
                                $categoryAddvertisement->whereIn('advertisings.door',$data['door']);

                            }
                            if(isset($data['condition']) && !empty($data['condition'])){
                                $categoryAddvertisement->whereIn('advertisings.condition',$data['condition']);
                        ...
                        //    
                            ...
                            return view('front.ajax_category',compact('url','categoryAddvertisement'));
            ...
            //   echo "<pre>";print_r( $categoryCount);die();
                    ...
                    //   dd($data['banner1']);
                        ...
                        $pageName = "Category";
                            ...
                            return view('front.category',$data,compact('pageName','nowdate','url','categoryAddvertisement','sellerType','transmission','doors','condition','aircondition','registerd'));
    ...
    if($request->ajax()){
        ...
        if($categoryCount > 0){
          ...
          $adds = Advertising::get();
            ...
            if (isset($data['sortajax']) && !empty($data['sortajax'])) {
                if ($data['sortajax']=="latest_listing") {
                  $categoryAddvertisement->orderBy('id','DESC');
                 
                }else if ($data['sortajax']=="old_listing") {
                    $categoryAddvertisement->orderBy('id','ASC');
