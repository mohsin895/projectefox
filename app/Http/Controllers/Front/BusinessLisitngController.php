...
class BusinessLisitngController extends Controller
{
    ...
    //  dd($request);
        ...
        $cart = Cart::content()->where('id',$id)->first();
            ...
            if(isset($cart)&& $cart != null){
                ...
                Cart::update($cart->rowId,['qty'=>$quantity , 'options'=>['logo'=>$logo ,'post_id'=>$post_id , 'total'=>$total]]);
            }else{
                $total = ((int)$quantity * (int)$totalPrice);
                Cart::add($id,$business_name,$quantity,$totalPrice,['logo'=>$logo ,'post_id'=>$post_id , 'total'=>$total]);
            }
    ...
    public function Order(){
        foreach(Cart::content() as $cart){
            ...
            $order->business_listing_id = $cart->id;
